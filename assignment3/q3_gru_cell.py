...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            b_r = tf.get_variable("b_r", shape=[self.state_size, ], initializer=tf.contrib.layers.xavier_initializer())
            ...
            b_z = tf.get_variable("b_z", shape=[self.state_size, ], initializer=tf.contrib.layers.xavier_initializer())
            ...
            b_o = tf.get_variable("b_o", shape=[self.state_size, ], initializer=tf.contrib.layers.xavier_initializer())

            z1 = tf.matmul(inputs, U_z) + tf.matmul(state, W_z) + b_z
            z_t = tf.nn.sigmoid(z1)

            z2 = tf.matmul(inputs, U_r) + tf.matmul(state, W_r) + b_r
            r_t = tf.nn.sigmoid(z2)

            z3 = tf.matmul(inputs, U_o) + r_t * tf.matmul(state, W_o) + b_o
            O_t = tf.nn.tanh(z3)
