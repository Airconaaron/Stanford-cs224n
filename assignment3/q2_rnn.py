...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if new_len < max_length:
            diff = max_length - new_len
            sentence = sentence + [zero_vector] * diff
            labels = labels + [zero_label] * diff
            mask = [True] * new_len + [False] * diff
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        #embedded_temp = self.pretrained_embeddings
        embedded = tf.nn.embedding_lookup(self.pretrained_embeddings, self.input_placeholder)
        embeddings = tf.reshape(embedded, shape=[-1, self.config.max_length, Config.n_features* Config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        # I presume the first dimension will get the batch size
        h = tf.zeros(name="h", shape=[ tf.shape(x)[0], self.config.hidden_size])
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_var, h = cell(x[:,time_step, :], h, scope="RNN")
                o_drop = tf.nn.dropout(o_var, dropout_rate)
                y_t = tf.matmul(o_drop, U) + b2
                preds.append(y_t)
                ### END YOUR CODE
