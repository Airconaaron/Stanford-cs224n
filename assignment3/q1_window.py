...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        new_list = []
        new_sentence = [start] * window_size + sentence + [end] * window_size
        #print new_sentence
        step = 2*window_size + 1
        for index in range(len(sentence)):
            #pos = step * index
            list = []
            temp_list = new_sentence[index:index+step]
            for item in temp_list:
                list = list + item
            temp = (list, labels[index])
            new_list.append(temp)
        windowed_data.extend(new_list)
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        #embed_temp = self.pretrained_embeddings
    ...
    def add_prediction_op(self):
        ...
        window_features = self.config.n_window_features
        ...
        #initialization
        W = tf.get_variable("W", shape=[window_features*embed_size, hidden_size], initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable("b1", shape=[1,hidden_size], initializer=tf.zeros_initializer)

        U = tf.get_variable("U", shape=[hidden_size, classes], initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.get_variable("b2", shape=[1,classes], initializer=tf.zeros_initializer)

        z1 = tf.matmul(x, W) + b1
        h = tf.nn.relu(z1)

        h_drop = tf.nn.dropout(h, keep_prob=keep_prob)
