...
def softmax(x):
    ...
    ### YOUR CODE HERE
    # numerical stability fixes refer to the minusing of shit
    max = tf.reduce_max(x, axis=1, keep_dims=True)
    x = tf.exp(x - max)
    out = x / tf.reduce_sum(x, axis=1, keep_dims=True) # so it was the stupid keep dims argument eh
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    y_logs = tf.log(yhat)
    y_multi = tf.multiply(y_logs, tf.to_float(y))
    out = -1 * tf.reduce_sum(y_multi) # because you're gonna collapse everything later on anyways
    ### END YOUR CODE
