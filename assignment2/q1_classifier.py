...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        # W = tf.Variable(initial_value=tf.zeros([self.config.n_features, self.config.n_classes]), name="weights", dtype=tf.float32)
        # b = tf.Variable(initial_value=tf.zeros([self.config.batch_size]), name="bias", dtype=tf.float32)
        #
        # z = tf.matmul(self.input_placeholder, W) + b
        #
        # pred = softmax(z)
        # return pred
        W = tf.Variable(tf.zeros([self.config.n_features, self.config.n_classes], tf.float32))
        b = tf.Variable(tf.zeros([self.config.n_classes], tf.float32))
        z = tf.matmul(self.input_placeholder, W) + b
        pred = softmax(z)
        ### END YOUR CODE
        return pred
