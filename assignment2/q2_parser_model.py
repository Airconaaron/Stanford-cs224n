...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.dropout_placeholder = tf.placeholder(dtype = tf.float32, shape = None)
        ### END YOUR CODE
    ...
    def add_embedding(self):
        ...
        lookup = tf.nn.embedding_lookup(self.pretrained_embeddings, self.input_placeholder)

        # lookup = tf.Print(lookup, [lookup, 'BOOBHOO'])
        #print lookup

        embeddings = tf.reshape(lookup, shape=[-1, self.config.n_features * self.config.embed_size])
        #print embeddings
        ### END YOUR CODE
        return embeddings
